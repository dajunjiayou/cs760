...
class ValueIteration(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for each_state in range(self.env.nS):
            ...
            # Store the old value of each state for later comparison
            old_value = self.V[each_state] 
            
            # Initialize an array to store the expected utility of each action
            ...
            # Update the value of each state to the maximum action value
            self.V[each_state] = np.max(action_values)
            
            # Track the largest change across all states
            delta = max(delta, abs(old_value - self.V[each_state]))
